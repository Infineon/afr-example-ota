...
parser.add_argument("--buildlocation", help="build folder location (can be relative)",default="../build/ota_cm4/CY8CPROTO-062-4343W/Debug", required=False)
...
class AWS_IoT_OTA:
    ...
    def BuildFirmwareFileNames(self):
        ...
        self.APP_NAME="ota_cm4_" + args.appversion + ".bin"
        self.APP_FULL_NAME=self.BUILD_PATH / Path(self.APP_NAME)
        self.BUILD_FILE_FULL_NAME=self.BUILD_PATH/Path("ota_cm4.bin")
    ...
    def __init__(self):
        boto3.setup_default_session(profile_name=str(args.profile))
        self.Session = boto3.session.Session(profile_name=str(args.profile))
        print("Session Profile: %s" % self.Session.profile_name)
        print("Available profiles: %s" % self.Session.available_profiles)
        print("Region: %s" % self.Session.region_name)
        if args.region=='':
